...
class TradingBot:
        ...
        try:
            ...
            for strategy in STRATEGIES:
                ...
                if direction:
                    ...
                    asyncio.create_task(self._execute_trade(signal, full_asset_name, last_candle.close))
                    break 
    ...
    async def _execute_trade(self, signal: TradeSignal, full_asset_name: str, entry_price: float):
    ...
    async def _check_and_process_single_trade(self, trade: ActiveTrade):
        try:
            ...
            await self.logger('INFO', f"[{trade.pair}] Expiração da ordem {trade.order_id}. A verificar preço de fecho...")
            # Pega na vela mais recente para obter o preço de fecho
            exit_candles = await self.exnova.get_historical_candles(trade.pair, 60, 1)
            
            result = "UNKNOWN"
            if exit_candles:
                exit_price = exit_candles[0].close
                if trade.direction == 'call' and exit_price > trade.entry_price:
                    result = "WIN"
                elif trade.direction == 'put' and exit_price < trade.entry_price:
                    result = "WIN"
                else:
                    result = "LOSS"
