...
class TradingBot:
    def __init__(self):
        # Inicializa os serviços de conexão
        ...
        # Gestão de Estado (Martingale e Operações)
    ...
    async def logger(self, level: str, message: str):
        """
        Envia logs para o console e para o Supabase de forma não bloqueante.
        """
        ts = datetime.utcnow().isoformat()
        print(f"[{ts}] [{level.upper()}] {message}", flush=True)
        try:
            # O insert_log é síncrono, então usamos to_thread para não parar o bot
            await asyncio.to_thread(self.supabase.insert_log, level, message)
        except Exception:
            pass # Ignora falhas de log para não derrubar o sistema

    async def _hourly_cycle_reset(self):
        """Reseta estatísticas de curto prazo a cada hora."""
        await self.logger('INFO', "CICLO HORÁRIO. A limpar estatísticas e blacklist temporária.")
    ...
    async def _daily_reset_if_needed(self):
        """Reseta metas diárias no início do dia UTC."""
        current_date_utc = datetime.utcnow().date()
        if self.last_daily_reset_date != current_date_utc:
            await self.logger('INFO', f"NOVO DIA ({current_date_utc}). Metas resetadas.")
            self.daily_wins = 0
            self.daily_losses = 0
            self.last_daily_reset_date = current_date_utc
            
            # Atualiza saldo inicial do dia
    ...
    async def run(self):
        ...
        if not await self.exnova.connect():
            await self.logger('ERROR', 'Falha inicial de conexão. O bot tentará recuperar no loop.')
        ...
        while self.is_running:
            try:
                ...
                # Verifica se a API ainda está conectada. Se não, força reconexão.
                if not await self.exnova.is_connected():
                    await self.logger('WARNING', 'Conexão perdida detetada! Tentando reconectar...')
                    if await self.exnova.connect():
                        await self.logger('SUCCESS', 'Reconectado com sucesso.')
                    else:
                        await self.logger('ERROR', 'Falha na reconexão. Aguardando 5s...')
                        await asyncio.sleep(5)
                        continue # Pula este ciclo e tenta de novo
                # ----------------------------------
                ...
                if status == 'RUNNING':
                    ...
                    # 2. Ciclo Normal de Análise
                    await self.trading_cycle()
                
                elif status != 'RUNNING':
                    if len(self.active_trading_pairs) > 0:
                        await self.logger('INFO', f"Pausado. Aguardando {len(self.active_trading_pairs)} trades ativos terminar.")
                    await asyncio.sleep(2) # Sleep maior quando pausado

                await asyncio.sleep(1) # Loop principal a cada 1s

            except Exception as e:
                print(f"[LOOP CRASH] {e}")
                traceback.print_exc()
                await asyncio.sleep(5) # Espera segura antes de reiniciar loop

    async def trading_cycle(self):
        now = datetime.utcnow()
        # Gatilho perto do fim do minuto (:50s)
        if now.second >= 50:
            if now.minute != self.last_analysis_minute:
                self.last_analysis_minute = now.minute
                
                # Verifica se é múltiplo de 5 min
                is_m5 = (now.minute + 1) % 5 == 0
                
                if is_m5:
                    await self.logger('INFO', f"Análise M5 iniciada...")
                    asyncio.create_task(self.run_analysis_for_timeframe(300, 5))
                else:
                    await self.logger('INFO', f"Análise M1 iniciada...")
    ...
    async def run_analysis_for_timeframe(self, timeframe_seconds: int, expiration_minutes: int):
        ...
        # Score de ativos (baseado em vitórias recentes)
        ...
        if len(self.active_trading_pairs) >= max_simultaneous: return

        # Lança análises em paralelo
        tasks = []
        for asset in target_assets:
            # Se já estamos operando este par, pula
        ...
        try:
            ...
            if expiration_minutes == 1:
                ...
                if m5_signal:
                    ...
                    if ti.check_rsi_condition(analysis_candles) == m5_signal['direction']: temp_conf.append("RSI_Condition")
            ...
            if final_direction:
                ...
                # Check triplo (se alguém entrou na frente)
                if base in self.active_trading_pairs: return
    ...
    async def _execute_martingale_trade(self, pair: str):
        ...
        # Calcula tempo para próxima vela
        now = datetime.utcnow()
    ...
    def _get_entry_value(self, asset: str, is_martingale: bool = False) -> float:
        ...
        # Se for gale, usa o nível atual. Se for entrada normal, nível é 0.
    ...
    async def _execute_and_wait(self, signal: TradeSignal, full_name: str, expiration: int):
        try:
            ...
            if not oid:
                ...
                # Se falhar entrada do gale, reseta para evitar prejuízo maior
            ...
            # Espera resultado (tempo da vela + buffer)
    ...
    async def process_trade_result(self, pair, full_name, result, sid, is_martingale, expiration, direction):
        ...
        if result == 'win':
            ...
            self.martingale_state[pair] = {'level': 0} # Reseta gale
