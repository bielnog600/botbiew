...
class TradingBot:
    ...
    async def _wait_for_entry_time(self):
        ...
        # Log para depuração do tempo de espera
        # await self.logger('DEBUG', f"A aguardar {wait_time}s para o ponto de entrada...")
        await asyncio.sleep(wait_time)


    async def _process_asset_task(self, full_asset_name: str):
        try:
            # FIX: A lógica de tempo agora é controlada por uma única função de sincronização.
            await self._wait_for_entry_time()
            
            clean_asset_name = full_asset_name.split('-')[0]
            
            await self.logger('INFO', f"[{full_asset_name}] Ponto de entrada atingido. A analisar vela pré-fecho...")
            ...
            for strategy in STRATEGIES:
                ...
                if direction:
                    await self.logger('SUCCESS', f"[{full_asset_name}] Sinal confirmado! Direção: {direction.upper()}")
                    ...
                    # A execução é imediata para garantir a entrada na viragem do minuto.
