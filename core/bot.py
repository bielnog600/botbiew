...
class TradingBot:
    ...
    def _execute_martingale_trade(self):
        ...
        self.logger('WARNING', f"MARTINGALE NÍVEL {current_level} PREPARADO para {trade_info['pair']}. Aguardando {wait_seconds:.2f}s para entrada na próxima vela.")
        time.sleep(wait_seconds)
    ...
    def _execute_and_wait(self, signal: TradeSignal, full_name: str, expiration_minutes: int):
        try:
            ...
            if not order_id:
                self.logger('ERROR', f"Falha ao executar a ordem para {full_name}. A corretora pode ter rejeitado a entrada.")
            ...
            if result == 'WIN':
                ...
                if pair in self.blacklisted_assets:
                    ...
                    self.logger('INFO', f"O par {pair} foi redimido e REMOVIDO da lista negra.")
            elif result == 'LOSS':
                self.daily_losses += 1
                ...
                if self.consecutive_losses[pair] >= 2:
                    ...
                    self.logger('ERROR', f"O par {pair} atingiu 2 derrotas consecutivas e foi COLOCADO na lista negra.")
                ...
                if self.bot_config.get('use_martingale', False):
                    ...
                    if current_level < max_levels:
                        ...
                        self.martingale_state[pair] = {'level': 0}
                        self.logger('ERROR', f"Nível máximo de Martingale atingido para {pair}. Ciclo de Martingale encerrado.")
