...
class SupabaseService:
    def __init__(self, url: str, key: str):
        ...
        # O loop de eventos é obtido uma vez para ser reutilizado
    ...
    async def get_bot_config(self) -> Dict:
        ...
        try:
            response = await self._execute_sync(self.client.from_('bot_config').select('*').eq('id', 1).single().execute)
    ...
    async def update_config(self, data: Dict) -> bool:
        ...
        try:
            await self._execute_sync(self.client.from_('bot_config').update(data).eq('id', 1).execute)
            return True
        except Exception as e:
    ...
    async def insert_log(self, level: str, message: str):
        ...
        try:
            await self._execute_sync(self.client.from_('bot_logs').insert({'level': level, 'message': message}).execute)
        except Exception as e:
    ...
    async def insert_trade_signal(self, signal: TradeSignal) -> Optional[int]:
        ...
        try:
            response = await self._execute_sync(self.client.from_('trade_signals').insert(signal.dict()).execute)
            if response.data:
    ...
    async def update_trade_result(self, signal_id: int, result: str, martingale_level: int):
        ...
        try:
            await self._execute_sync(self.client.from_('trade_signals').update({
                'result': result,
