...
def is_market_indecisive(velas, p):
    ...
    last_candles = velas[-candles_to_check:]
    indecisive_candles_found = 0
    for vela in last_candles:
        range_total = vela['high'] - vela['low']
        if range_total == 0: indecisive_candles_found += 1; continue
        corpo = abs(vela['open'] - vela['close'])
        if (corpo / range_total) <= body_max_ratio: indecisive_candles_found += 1
    if indecisive_candles_found >= min_indecisive_count: return True
    return False
...
def exibir_placar(state, config):
    # ... (código mantido)
    print(y + "\n" + "="*12 + " PLACAR " + "="*12)
    print(f"{g}>> WIN de Primeira: {w}{state.win_count}")
    if config['usar_mg']:
        ...
        try:
            ...
            if segundo_atual >= 50 and not analise_feita_para_este_minuto and not state.is_trading:
                ...
                if direcao_final:
                    ...
                    # ### AJUSTE 3 ### Passa o momento exato para a thread
                    ...
                    # ### AJUSTE 4 ### Inicia a thread imediatamente com o alvo
